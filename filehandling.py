...
letter = sum(c.isalpha() for c in content)
...
file = open("copy_notes.txt", "w")
file.write(content)
file.close()

# Q12. Write a program using with open(...) (context manager) to read a file — explain why with is preferred over open()/close().

with open("notes.txt", "r") as file:
    with open("copy_notes.txt", "w") as copy_file:
        content = file.read()
...
keyword = "hai"
with open("notes.txt", "r") as file:
    with open("keyword_lines.txt", "w") as keyword_file:
        for line in file:
            if keyword in line:
